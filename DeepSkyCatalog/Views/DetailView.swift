...
struct DetailView: View {
            ...
            VStack(spacing: 10) {
                ...
                VStack() {
                    TargetAltitudeChart(target: target)
                        .padding()
                    HStack {
                        if let eventOrder = getNextOrder(for: target) {
                ...
                HStack {
                    ...
                    VStack {
                        Text("Visibility Score: \((target.getVisibilityScore(at: location, on: date)).percent())")
                            .foregroundColor(.secondary)
                        Text("Meridian Score: \((target.getMeridianScore(at: location, on: date)).percent())")
                            .foregroundColor(.secondary)
                    }
                }
    ...
    func getNextOrder(for target: DeepSkyTarget) -> [EventInfo]? {
        do {
            let array = [EventInfo(event: .rise, date: try target.getNextInterval(at: location, on: date).start, error: nil), EventInfo(event: .meridian, date: try target.getNextMeridian(at: location, on: date), error: nil), EventInfo(event: .set, date: try target.getNextInterval(at: location, on: date).end, error: nil)]
